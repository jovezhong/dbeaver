...
import org.jkiss.dbeaver.ext.timeplus.model.TimeplusTupleType;
import org.jkiss.dbeaver.ext.timeplus.model.data.TimeplusMapValue;
import org.jkiss.dbeaver.ext.timeplus.model.data.TimeplusTupleValue;
...
public class TimeplusTypeParser {
    private static final Log log = Log.getLog(TimeplusTypeParser.class);
    ...
    public static Object makeValue(@NotNull DBCSession session, @NotNull String typeName, @Nullable Object object) throws DBException {
        final DBSDataType type = getType(session.getProgressMonitor(), (TimeplusDataSource) session.getDataSource(), typeName);
    ...
    public static Object makeValue(@NotNull DBCSession session, @NotNull DBSTypedObject type, @Nullable Object object) throws DBException {
        ...
        if (type instanceof TimeplusMapType map && ENABLE_COMPLEX_TYPE_PARSING) {
            return new TimeplusMapValue((TimeplusDataSource) session.getDataSource(), map, ((Map<?, ?>) object));
        } else if (type instanceof TimeplusTupleType tuple && ENABLE_COMPLEX_TYPE_PARSING) {
            final Object[] values;
            ...
            return new TimeplusTupleValue(session.getProgressMonitor(), tuple, values);
        } else {
            return object;
        }
    }
    ...
    @Nullable
    private static DBSDataType getArrayType(@NotNull DBRProgressMonitor monitor, @NotNull TimeplusDataSource dataSource, ArrayTypeContext type) throws DBException {
        ...
        return new TimeplusArrayType(dataSource, componentType);
    }

    @Nullable
    private static DBSDataType getTupleType(@NotNull DBRProgressMonitor monitor, @NotNull TimeplusDataSource dataSource, @NotNull TupleTypeContext context) throws DBException {
        ...
        return new TimeplusTupleType(dataSource, elements);
    }

    @Nullable
    private static DBSDataType getMapType(@NotNull DBRProgressMonitor monitor, @NotNull TimeplusDataSource dataSource, @NotNull MapTypeContext context) throws DBException {
        ...
        return new TimeplusMapType(dataSource, keyType, valueType);
    }
