...
import org.jkiss.dbeaver.ext.timeplus.TimeplusConstants;
import org.jkiss.dbeaver.ext.timeplus.TimeplusTypeParser;
...
public class TimeplusDataSource extends GenericDataSource {
    ...
    private static final Log log = Log.getLog(TimeplusDataSource.class);
    ...
    public TimeplusDataSource(DBRProgressMonitor monitor, DBPDataSourceContainer container, GenericMetaModel metaModel)
        throws DBException
    {
    ...
    List<TimeplusTableEngine> getTableEngines() {
        return engineCache.getCachedObjects();
    }

    TimeplusTableEngine getEngineByName(@NotNull String engineName) {
    ...
    private void initSSL(DBRProgressMonitor monitor, Properties properties, DBWHandlerConfiguration sslConfig) throws DBException {
        ...
        properties.put(TimeplusConstants.SSL_PARAM, "true");
        try {
            if ("com_clickhouse".equals(getContainer().getDriver().getId())) {
                if (DBWorkbench.isDistributed() || DBWorkbench.getPlatform().getApplication().isMultiuser()) {
                    ...
                    if (!CommonUtils.isEmpty(clientCertProp)) {
                        properties.put(TimeplusConstants.SSL_PATH, saveCertificateToFile(clientCertProp));
                    }
                    ...
                    if (!CommonUtils.isEmpty(clientKeyProp)) {
                        properties.put(TimeplusConstants.SSL_KEY_PASSWORD, saveCertificateToFile(clientKeyProp));
                    }
                } else {
                    properties.put(TimeplusConstants.SSL_PATH,
                        sslConfig.getStringProperty(SSLHandlerTrustStoreImpl.PROP_SSL_CLIENT_CERT)
                    );
                    properties.put(TimeplusConstants.SSL_KEY_PASSWORD,
                        sslConfig.getStringProperty(SSLHandlerTrustStoreImpl.PROP_SSL_CLIENT_KEY)
                    );
                }
                properties.put(TimeplusConstants.SSL_MODE,
                    sslConfig.getStringProperty(TimeplusConstants.SSL_MODE_CONF)
                );
            } else {
                // Old clickhouse used lowercase for sslmode, we should send it in the lowercase
                String mode = sslConfig.getStringProperty(TimeplusConstants.SSL_MODE_CONF);
                if (mode != null) {
                    properties.put(TimeplusConstants.SSL_MODE, mode.toLowerCase());
                }
            }
            if (DBWorkbench.isDistributed() || DBWorkbench.getPlatform().getApplication().isMultiuser()) {
                ...
                if (!CommonUtils.isEmpty(caCertProp)) {
                    properties.put(TimeplusConstants.SSL_ROOT_CERTIFICATE, saveCertificateToFile(caCertProp));
                }
            } else {
                properties.put(TimeplusConstants.SSL_ROOT_CERTIFICATE,
    ...
    public DBSDataType resolveDataType(@NotNull DBRProgressMonitor monitor, @NotNull String typeFullName) throws DBException {
        ...
        if (TimeplusTypeParser.isComplexType(typeFullName)) {
            final DBSDataType type = TimeplusTypeParser.getType(monitor, this, typeFullName);
            if (type != null) {
                return type;
            }
        }
    ...
    static class TableEnginesCache extends JDBCObjectCache<TimeplusDataSource, TimeplusTableEngine> {
        ...
        protected JDBCStatement prepareObjectsStatement(
            ...
            @NotNull TimeplusDataSource clickhouseDataSource) throws SQLException {
        ...
        @Nullable
        @Override
        protected TimeplusTableEngine fetchObject(
            @NotNull JDBCSession session,
            @NotNull TimeplusDataSource clickhouseDataSource,
            @NotNull JDBCResultSet dbResult) {

            final String engineName = JDBCUtils.safeGetString(dbResult, 1);
            if (CommonUtils.isNotEmpty(engineName)) {
                return new TimeplusTableEngine(engineName, clickhouseDataSource);
            }
            return null;
        }
    }
