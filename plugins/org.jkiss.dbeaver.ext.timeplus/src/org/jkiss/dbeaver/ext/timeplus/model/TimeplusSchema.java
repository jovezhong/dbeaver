 ...
 * ClickhouseSchema
 */
public class TimeplusSchema extends GenericSchema implements DBPObjectStatisticsCollector
{
    private boolean hasStatistics = true;

    public TimeplusSchema(@NotNull GenericDataSource dataSource, @Nullable GenericCatalog catalog, @NotNull String schemaName) {
    ...
    @Override
    public List<TimeplusTable> getPhysicalTables(DBRProgressMonitor monitor) throws DBException {
        return (List<TimeplusTable>) super.getPhysicalTables(monitor);
    }

    @Override
    public List<TimeplusTable> getTables(DBRProgressMonitor monitor) throws DBException {
        return (List<TimeplusTable>) super.getTables(monitor);
    }
                    ...
                    "GROUP BY table"))
            {
                ...
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    while (dbResult.next()) {
                        ...
                        if (table instanceof TimeplusTable) {
                            ((TimeplusTable)table).fetchStatistics(dbResult);
                        }
                    }
                }
