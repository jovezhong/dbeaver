 ...
 */
package org.jkiss.dbeaver.ext.timeplus.model.data;
...
public class TimeplusValueHandlerProvider implements DBDValueHandlerProvider {
    @Override
    public DBDValueHandler getValueHandler(DBPDataSource dataSource, DBDFormatSettings preferences, DBSTypedObject type) {
        ...
        if ("enum8".equals(lowerTypeName) || "enum16".equals(lowerTypeName)) {
            return TimeplusEnumValueHandler.INSTANCE;
        } else if (dataKind == DBPDataKind.ARRAY) {
            return TimeplusArrayValueHandler.INSTANCE;
        } else if (dataKind == DBPDataKind.STRUCT) {
            return TimeplusStructValueHandler.INSTANCE;
        } else if ("bool".equals(lowerTypeName)) {
            return TimeplusBoolValueHandler.INSTANCE;
            ...
            if (lowerTypeName.contains("int128") || lowerTypeName.contains("int256")
                ...
                return new TimeplusBigNumberValueHandler(type, preferences);
            } else {
                return new JDBCNumberValueHandler(type, preferences);
            }
        } else if (TimeplusConstants.DATA_TYPE_IPV4.equals(lowerTypeName) || TimeplusConstants.DATA_TYPE_IPV6.equals(lowerTypeName)) {
            return TimeplusInetTypeValueHandler.INSTANCE;
        } else if (dataKind == DBPDataKind.DATETIME) {
            return new TimeplusDateTimeValueHandler(preferences);
        }
        return null;
    }
}
